...
## /handoff vs full session closeout

- **`/handoff`** — dev work pre-`/clear`. Chat-only. Covers background processes, dev servers, verification commands, pick-up pointer. No file writes. No commit. No CRM writes.
- **Full session closeout** — business/ops session end. Writes to meta and state files. Commits and pushes. Updates CRM tasks.

If the session was a dev session on a specific project, use `/handoff`. If it was a planning or business session that produced tracked decisions, use the closeout skill instead.
